...
const PortfolioPage = () => {
  ...
  const selectedProject = selectedItem !== null ? portfolioItems.find(item => item.id === selectedItem) : null;
  return <div className="min-h-screen relative bg-space-dark">
      <Navigation />
          ...
          <div className="container mx-auto px-6">
            <div className="flex justify-center mb-12">
              <nav className="glass-card inline-flex p-1 rounded-full">
                <button onClick={() => setFilter('all')} className={cn("px-5 py-2 rounded-full text-sm font-medium transition-colors", filter === 'all' ? "bg-gradient-to-r from-space-turquoise to-space-purple text-white" : "text-white hover:text-space-turquoise")}>
                  הכל
                </button>
                <button onClick={() => setFilter('web')} className={cn("px-5 py-2 rounded-full text-sm font-medium transition-colors", filter === 'web' ? "bg-gradient-to-r from-space-turquoise to-space-purple text-white" : "text-white hover:text-space-turquoise")}>
                  אתרים
                </button>
                <button onClick={() => setFilter('logo')} className={cn("px-5 py-2 rounded-full text-sm font-medium transition-colors", filter === 'logo' ? "bg-gradient-to-r from-space-turquoise to-space-purple text-white" : "text-white hover:text-space-turquoise")}>
                  לוגואים
                </button>
                <button onClick={() => setFilter('landing')} className={cn("px-5 py-2 rounded-full text-sm font-medium transition-colors", filter === 'landing' ? "bg-gradient-to-r from-space-turquoise to-space-purple text-white" : "text-white hover:text-space-turquoise")}>
                  דפי נחיתה
                </button>
              </nav>
            </div>
            
            {selectedItem === null ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {filteredItems.map(item => <div key={item.id} className="glass-card overflow-hidden group cursor-pointer transition-transform hover:scale-[1.02] relative" onClick={() => setSelectedItem(item.id)}>
                  {/* עטיפה מיוחדת על התמונה עם אפקטי glow */}
                  <div className="relative w-full h-64 overflow-hidden">
                    {/* תמונה */}
                    <img src={item.image} alt={item.title} className="w-full h-full object-cover transition-all duration-500 group-hover:scale-110 group-hover:brightness-75" />
              ...
              </div> : <div className="glass-card p-6 md:p-10">
                <button onClick={() => setSelectedItem(null)} className="text-space-turquoise hover:text-white transition-colors flex items-center mb-8">
                ...
                {selectedProject && <div>
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2 mb-8">
                          {selectedProject.tags.map((tag, index) => <span key={index} className="text-sm py-1 px-3 rounded-full bg-space-dark/70 text-gray-300">
                              {tag}
                            </span>)}
                        </div>
                        
                        <a href="#" className="inline-flex items-center gap-2 bg-gradient-to-r from-space-turquoise to-space-purple text-white font-medium py-2 px-6 rounded-lg transition-transform hover:scale-105">
                          צפה בפרויקט החי
                    ...
                    <div className="mt-16">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {portfolioItems.filter(item => item.id !== selectedProject.id && item.category === selectedProject.category).slice(0, 3).map(item => <div key={item.id} className="glass-card overflow-hidden cursor-pointer transition-transform hover:scale-[1.02]" onClick={() => setSelectedItem(item.id)}>
        ...
        <section className="py-20 relative">
          ...
          <div className="container mx-auto px-6 relative z-10">
            <div className="glass-card p-10 text-center max-w-4xl mx-auto">
              ...
              <Link to="/contact" className="bg-gradient-to-r from-space-turquoise to-space-purple text-white font-medium py-3 px-8 rounded-lg text-lg transition-transform hover:scale-105 shadow-lg">
                נדבר על הפרויקט שלך
              </Link>
            </div>
          </div>
